...
import { getProducts, getCustomers, createTransactionAction, getSettingsAction } from "@/app/actions";
...
export default function Cashier() {
  ...
  return (
    <div className="grid grid-cols-12 gap-6 h-full overflow-hidden">
      {/* Left Column: Cart & Product Search */}
      <div className="col-span-12 lg:col-span-8 flex flex-col h-full overflow-hidden">
        ...
        <div className="relative shrink-0 mb-4">
          ...
          <input
            ...
            className="w-full bg-[#18181b] border border-zinc-600 rounded-2xl py-3.5 pl-12 pr-4 text-zinc-100 placeholder-zinc-400 focus:outline-none focus:border-primary text-sm font-semibold transition-all shadow-inner"
          />
        </div>

        {/* Side-by-Side Cart and Catalog Panels */}
        <div className="flex-1 flex gap-4 min-h-0 overflow-hidden">
          {/* Left Panel: Cart Items (45%) */}
          <div className="w-[45%] flex flex-col glass-card rounded-2xl overflow-hidden min-h-0">
            <div className="h-12 bg-zinc-900 border-b border-zinc-700 flex items-center px-4 shrink-0 justify-between">
              <span className="text-xs font-bold text-zinc-200 uppercase tracking-wider">Keranjang ({totalItemsCount})</span>
              {items.length > 0 && (
                <button 
                  onClick={clearCart}
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center text-zinc-400 p-4">
                  <ShoppingCart className="w-8 h-8 mb-2 text-zinc-300" />
                  <p className="text-xs font-bold text-zinc-200">Keranjang kosong</p>
                  <p className="text-[10px] text-zinc-400 mt-0.5">Pilih produk di sebelah kanan</p>
                </div>
              ) : (
                items.map((item) => {
                     ...
                     <div key={item.productId} className="flex flex-col bg-zinc-950/60 p-2.5 rounded-xl border border-zinc-700 space-y-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="w-5.5 h-5.5 rounded-md bg-zinc-800 hover:bg-zinc-700 border border-zinc-600 flex items-center justify-center text-zinc-200 cursor-pointer"
                        >
                      ...
                      <span className="text-xs font-extrabold text-zinc-100">Rp {item.subtotal.toLocaleString("id-ID")}</span>
                    </div>
                  </div>
                );
              })
          ...
          {/* Right Panel: Catalog Grid (55%) */}
          <div className="w-[55%] flex flex-col glass-card rounded-2xl overflow-hidden min-h-0">
            <div className="h-12 bg-zinc-900 border-b border-zinc-700 flex items-center px-4 shrink-0 justify-between">
              <span className="text-xs font-bold text-zinc-200 uppercase tracking-wider">Katalog Barang</span>
              <span className="text-[10px] text-zinc-300 font-bold">{dbProducts.length} produk</span>
            </div>
            <div className="flex-1 overflow-y-auto p-3 grid grid-cols-2 gap-2.5">
              {dbProducts.filter(p => p.isActive && (searchQuery === "" || p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.barcode.includes(searchQuery))).map((p) => {
                ...
                return (
                  <button
                    key={p.id}
                    onClick={() => addItem(p, 1)}
                    className="text-left bg-zinc-950/60 hover:bg-zinc-800/80 p-2.5 rounded-xl border border-zinc-700 hover:border-zinc-500 transition-all flex flex-col justify-between h-24 relative overflow-hidden group cursor-pointer"
                  >
                    {hasDiscount && (
                      <div className="absolute top-0 right-0 bg-red-600 text-white font-extrabold text-[8px] px-2 py-0.5 rounded-bl-lg z-10 shadow-sm animate-pulse">
                        -{p.discountPercent}%
                      </div>
                    )}
                    <div>
                      <h6 className="text-[11px] font-extrabold text-zinc-100 group-hover:text-primary transition-colors truncate pr-8">{p.name}</h6>
                      <span className={`text-[8px] px-1 py-0.2 rounded border inline-block mt-1 font-bold ${
                        isLow ? "bg-red-500/10 border-red-500/30 text-red-400 animate-pulse" : "bg-zinc-800 border-zinc-600 text-zinc-200"
                      }`}>
                        Stok: {p.stock}
                      </span>
                    </div>
                    <div className="flex justify-between items-end w-full mt-2">
                      <div className="flex flex-col">
                        {hasDiscount ? (
                          <>
                            ...
                            <span className="text-[8px] text-zinc-400 line-through font-mono mt-0.5">
                      ...
                      <span className="text-[9px] text-zinc-300 font-bold group-hover:text-zinc-100">Tambah +</span>
                    </div>
                  </button>
                );
              })}
      ...
      <div className="col-span-12 lg:col-span-4 flex flex-col h-full gap-6">
        ...
        <div className="glass-card rounded-2xl p-5 shrink-0 space-y-4">
          ...
          <div className="flex gap-2">
            <select
              ...
              className="flex-1 bg-[#18181b] border border-zinc-600 rounded-xl px-3 py-2 text-xs text-zinc-250 focus:outline-none focus:border-primary font-bold"
            >
          ...
          {activeCustomer && (
            <div className="space-y-2">
              <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 flex justify-between items-center">
                <div>
                  <p className="text-[10px] text-zinc-300 uppercase font-bold tracking-wider">Poin Loyalitas</p>
                  <p className="text-xs font-bold text-zinc-100 mt-0.5">{activeCustomer.name}</p>
                </div>
                <span className="text-xs font-black text-emerald-400 bg-emerald-500/10 px-2.5 py-1.5 rounded-lg border border-emerald-500/35">
              ...
              {pointRedeemValue > 0 && activeCustomer.points > 0 && (
                <div className="bg-emerald-500/5 border border-emerald-500/25 rounded-xl p-3 space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider">Tukarkan Poin</span>
                    <span className="text-[9px] text-zinc-400 font-medium">1 Poin = Rp {pointRedeemValue}</span>
                  </div>
                  <div className="flex gap-2">
                    <input
                      type="number"
                      ...
                      className="flex-1 bg-[#18181b] border border-zinc-650 rounded-lg py-1.5 px-3 text-xs text-zinc-100 focus:outline-none focus:border-primary font-bold font-mono"
                    ...
                    <button
                      ...
                      className="bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/25 text-emerald-400 text-[10px] font-bold px-2 py-1.5 rounded-lg cursor-pointer transition-colors"
                    >
                      Semua
                    </button>
                  </div>
                  {pointsDiscount > 0 && (
                    <p className="text-[9px] text-emerald-400 font-semibold text-right">
        ...
        {heldCarts.length > 0 && (
          <div className="glass-card rounded-2xl p-5 shrink-0">
            <span className="text-xs font-bold text-zinc-200 uppercase tracking-wider flex items-center gap-1.5 mb-3">
              <FolderLock className="w-4 h-4 text-amber-400" />
              Transaksi Ditangguhkan ({heldCarts.length})
            </span>
            <div className="space-y-2 max-h-32 overflow-y-auto">
              {heldCarts.map((hc) => (
                <div key={hc.id} className="flex justify-between items-center bg-amber-500/10 border border-amber-500/20 p-2.5 rounded-xl">
                  <div>
                    <p className="text-[11px] font-black text-amber-400">Draft #{hc.id}</p>
                    <p className="text-[9px] text-zinc-300 font-medium">{hc.items.length} item • Pkl {hc.heldAt}</p>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => deleteHeldCart(hc.id)}
                      className="text-[10px] text-zinc-350 hover:text-red-400 font-bold cursor-pointer"
                    >
                      Batal
                    </button>
                    <button
                      onClick={() => resumeCart(hc.id)}
                      className="bg-amber-500/20 border border-amber-500/40 text-amber-300 text-[10px] font-bold px-2 py-1 rounded-md hover:bg-amber-500/30 transition-all cursor-pointer"
                    >
                      Panggil
                    </button>
                  </div>
        ...
        <div className="glass-card rounded-2xl p-6 flex-1 flex flex-col justify-between">
          <div className="space-y-4">
            {/* Total Display */}
            <div className="bg-[#18181b] border border-zinc-650 rounded-2xl p-5 text-right relative overflow-hidden">
              <span className="text-[10px] text-zinc-300 uppercase font-bold tracking-wider">Total Pembayaran</span>
              <h2 className="text-3xl font-extrabold text-white mt-1 tabular-nums">
                Rp {totalPaid.toLocaleString("id-ID")}
              </h2>
              <div className="flex flex-col gap-0.5 mt-1.5 text-right">
                {discount > 0 && (
                  <p className="text-[10px] text-red-400 font-bold">
                    Diskon Manual: -Rp {discount.toLocaleString("id-ID")}
                  </p>
                )}
                {grosirDiscount > 0 && (
                  <p className="text-[10px] text-amber-400 font-bold">
                    Promo Grosir ({grosirDiscountRate}%): -Rp {grosirDiscount.toLocaleString("id-ID")}
                  </p>
                )}
                {memberDiscount > 0 && (
                  <p className="text-[10px] text-violet-400 font-bold">
                    Diskon Member ({memberDiscountRate}%): -Rp {memberDiscount.toLocaleString("id-ID")}
                  </p>
                )}
                {pointsDiscount > 0 && (
                  <p className="text-[10px] text-emerald-400 font-bold">
                    Diskon Poin Member: -Rp {pointsDiscount.toLocaleString("id-ID")}
                  </p>
                )}
                {enableTax && tax > 0 && (
            ...
            {/* Discount Inputs */}
            <div>
              <label className="text-xs text-zinc-300 font-bold flex items-center gap-1.5 mb-2">
                <BadgePercent className="w-4 h-4 text-zinc-300" />
                Diskon Transaksi (Rupiah)
              </label>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-300 text-xs font-bold">Rp</span>
                <input
                  type="number"
                  value={discount || ""}
                  onChange={(e) => setDiscount(parseFloat(e.target.value) || 0)}
                  className="w-full bg-[#18181b] border border-zinc-600 rounded-xl py-2.5 pl-10 pr-4 text-xs text-zinc-100 focus:outline-none focus:border-primary font-bold transition-all placeholder-zinc-500"
                  placeholder="0"
                />
              </div>
            </div>
          ...
          <div className="space-y-4 pt-6">
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3 px-4 rounded-xl border border-zinc-600 hover:border-zinc-500 text-zinc-300 hover:text-zinc-100 disabled:opacity-40 disabled:pointer-events-none text-xs font-bold transition-all duration-200 cursor-pointer"
              >
                Tangguhkan
              </button>
              ...
              <button
                ...
                disabled={items.length === 0}
                className="flex-2 py-3 px-5 rounded-xl bg-primary text-white hover:bg-primary/95 shadow-lg shadow-primary/20 hover:shadow-primary/30 disabled:opacity-40 disabled:pointer-events-none text-xs font-bold transition-all duration-200 cursor-pointer"
              >
                Bayar Belanja (F8)
              </button>
            </div>
